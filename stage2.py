...
from database import split_into_chunks , MCQ_STAGE_2
...
image = pygame.image.load("./Assets/tiles/Purple.png")
...
for val in questions_places:
    question = database.LESSON_STAGE_2[inx]
    inx += 1
...
def coverBackgroundStage2(WIN , player_position_X , player_position_Y , keys , mouse_x , mouse_y , button_clicked):
    ...
    if constants.BANNER_DISPLAY:
        ...
        draw_text(WIN , "Stage Two" , constants.BANNER_CENTER_X + 180 , constants.BANNER_CENTER_Y + 30 , 2)
        ...
        draw_text(WIN , "Stage Two" , 10 , 10 , 1.5)
        draw_text(WIN , "Five Question" , 18 , 30 , 0.8)

    text_surface = HEADING.render("Functions" , True , (0,0,0))
    ...
    if player_position_Y <= gate.y + gate.height and player_position_X + constants.PLAYER_WIDTH >= gate.x and player_position_X <= gate.x + gate.width:
        ...
        if not five_question:
            five_question = random.sample(MCQ_STAGE_2 , 5)

        if constants.EXAM_2 == False:
            examForm(WIN , five_question , mouse_x , mouse_y , "form2")

    if constants.EXAM_2 == True:
        constants.COMPLETED_STAGE2 = True
